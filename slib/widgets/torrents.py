...
def torrentsList(torrents_dir_path, link_prefix) :
	...
	size = 0
	rows_list = []
	for (torrent_file_name, meta_dict) in sorted(tools.torrents.torrents(torrents_dir_path).items(), key=( lambda arg : arg[0] )) :
		torrent_file_path = os.path.join(link_prefix, torrent_file_name)
		torrent_size = tools.torrents.torrentSize(meta_dict)
		size += torrent_size
		rows_list.append([
				str(len(rows_list) + 1),
